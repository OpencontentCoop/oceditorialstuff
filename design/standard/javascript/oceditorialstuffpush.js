jQuery(document).ready(function ($) {
    ...
    $("#ngpush-list textarea[class^='maxlength']").keyup(function () {
        maxlength(this);
    });
    $("#ngpush-list textarea[class^='maxlength']").each(function () {
        maxlength(this);
    });
...
//	});
    ...
    $('#ngpush-list form input[type=reset]').click(function (e) {
        ...
        $(this).parents('form').find('textarea.maxlength').each(function () {
            maxlength($(this).get(0));
        });
    });
...
function ngpush_process_account(account_type, account_id) {
    switch (account_type) {
        case 'twitter':
            ...
            jQuery.ez('oceditorialstuffpush::push',
                {
                    factoryIdentifier: ngpush_factory_identifier,
                    nodeID: ngpush_node_id,
                    accountID: account_id,
                    tw_status: tw_status
                },
                function (data) {
                    ngpush_process_response(data, account_id);
                });
        ...
        case 'facebook_event':
            ...
            jQuery.ez('oceditorialstuffpush::push', {
                ...
                accountID: account_id,
                fb_name: fb_name,
                fb_start_time: fb_start_time,
                fb_end_time: fb_end_time
            }, function (data) {
